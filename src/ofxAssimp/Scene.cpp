...
bool Scene::load(const ofBuffer& buffer, bool optimize,
...
Node::Ref Scene::nodeSetupVisiter(Scene* s, aiNode* node) {
	Node::Ref n = Node::Ref(new Node(s, node));
	s->nodes[n->getName()] = n;
	s->nodeNames.push_back(n->getName());
	
	for (int i = 0; i < node->mNumChildren; i++) {
		aiNode* o = node->mChildren[i];
		Node::Ref child = nodeSetupVisiter(s, o);
		child->setParent(*n);
	}
